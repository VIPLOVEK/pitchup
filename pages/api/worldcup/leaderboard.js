...
export default async function handler(req, res) {
  ...
  const { data: preds } = await db.from('wc_predictions').select('player_name, player_id, prediction, is_correct')
  if (!preds) return res.status(200).json([])

  const map = {}
  for (const p of preds) {
    const key = p.player_name
    if (!map[key]) map[key] = { name: p.player_name, playerId: p.player_id, total: 0, correct: 0 }
    map[key].total++
