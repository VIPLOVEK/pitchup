...
export default async function handler(req, res) {
  ...
  const { title, location, slots, minPlayers, maxPlayers, visibility, groupIds, notes, gameType, opponent, noTeamSplit, cutoffHours, autoLockHours, pitchFee, splitByClub, clubNameA, clubNameB } = req.body
  ...
  try {
    ...
    const { data, error } = await db
      ...
      .insert({
        ...
        team_a_name: (splitByClub && clubNameA?.trim()) ? clubNameA.trim() : teamAName,
        team_b_name: (splitByClub && clubNameB?.trim()) ? clubNameB.trim() : teamBName,
      })
      .select()
      .single()
