...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    try {
      ...
      if (action === 'setDefensive') {
        const { defensive } = req.body
        if (poll.status !== 'confirmed') return res.status(400).json({ error: 'Game must be confirmed first' })
        if (!Array.isArray(defensive)) return res.status(400).json({ error: 'defensive must be an array' })
        const { data, error } = await db
          .from('polls').update({ defensive, version: poll.version + 1 }).eq('id', id).select().single()
        if (error) throw error
        return res.status(200).json(data)
      }
