...
export async function sendWhatsAppAnnouncement({ poll, teamA, teamB, gameTime }) {
  ...
  const message = [
    ...
    ``,
    `🟦 *${poll.team_a_name || 'Team A'}:* ${teamANames}`,
    `🟥 *${poll.team_b_name || 'Team B'}:* ${teamBNames}`,
    ``,
...
export async function sendWhatsAppReminder({ poll }) {
  ...
  const needed = poll.min_players - players.length

  const message = [
    `⏰ *Reminder: "${poll.title}"*`,
    ``,
    `Still need ${needed} more player${needed === 1 ? '' : 's'} to confirm the game (${players.length}/${poll.min_players}).`,
...
export async function sendWhatsAppCancellation({ poll }) {
  ...
  const message = [
    ...
    ``,
    `${players.length}/${poll.min_players} minimum players joined.`,
    `📍 ${poll.location}`,
    ``,
    `Maybe next time! 🏃`,
  ].join('\n')
